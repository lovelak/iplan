...
<html lang="en">
    ...
    <script type="text/javascript">
        function changeYear(selectValue) {
            ...
            $("#projectImplement").load("ajax/getProjectImplementDistrictNew.php?y=" + selectValue + "&p=" + provinceSelected.value + "&r=" + roundSelected.value + "&a=" + districtSelected.value);
        ...
        function changeProvince(selectValue2) {
            ...
            var roundSelected = document.getElementById('selectRound');
            var districtSelected = document.getElementById('selectDistrict');
            $("#projectImplement").load("ajax/getProjectImplementDistrictNew.php?p=" + selectValue2 + "&y=" + yearSelected.value + "&r=" + roundSelected.value + "&a=" + districtSelected.value);
        }
        function changeRound(selectValue3) {
            var provinceSelected = document.getElementById('selectProvince');
            var yearSelected = document.getElementById('yearProject');
            var districtSelected = document.getElementById('selectDistrict');
            $("#projectImplement").load("ajax/getProjectImplementDistrictNew.php?r=" + selectValue3 + "&y=" + yearSelected.value + "&p=" + provinceSelected.value + "&a=" + districtSelected.value);
        }
    </script>
...
<select name ="province" id="selectProvince" onchange="changeProvince(this.value)" required>
    ...
    <?php
    $a = $_SESSION["ss_area"]; // ตัวแปรภาค ******** เอาไว้ เลือกจังหวัดในภาคมาแสดง
    require 'include/ConnectionDb.php';
    $sql="select * from provinces WHERE area = $a";
...
<select name = "district" id="selectDistrict" hidden="true">
   <option value="<?php echo $_SESSION["ss_area"]; ?>" selected>รหัสภาค</option>
</select>
